...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo " SELECT movies.title , movies.year, tags.tag FROM tags INNER JOIN movies ON tags.movie_id = movies.id WHERE movies.year IS NOT NULL  ORDER BY movies.year, movies.title, tags.tag LIMIT 40;"
echo " "
